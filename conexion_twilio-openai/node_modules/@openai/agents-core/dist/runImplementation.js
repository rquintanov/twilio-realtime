    ...
    async function runSingleTool(toolRun) {
        ...
        return (0, createSpans_1.withFunctionSpan)(async (span) => {
            ...
            try {
                ...
                const result = await toolRun.tool.invoke(state._context, toolRun.toolCall.arguments, { toolCall: toolRun.toolCall });
                ...
                return {
                    ...
                    tool: toolRun.tool,
                    output: result,
                    runItem: new items_1.RunToolCallOutputItem(getToolCallOutputItem(toolRun.toolCall, result), agent, result),
                };
            }
            catch (error) {
                span.setError({
...
async function checkForFinalOutputFromTools(agent, toolResults, state) {
    ...
    const interruptions = toolResults
        .filter((r) => r.runItem instanceof items_1.RunToolApprovalItem)
        .map((r) => r.runItem);
